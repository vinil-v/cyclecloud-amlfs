...
echo "[INFO] Starting unmount process for $MOUNTPOINT" | tee -a "$LOGFILE"

while mountpoint -q $MOUNTPOINT; do
    echo "[WARNING] $MOUNTPOINT is still mounted. Checking processes..." | tee -a "$LOGFILE"
    /sbin/fuser -vm $MOUNTPOINT 2>&1 | tee -a "$LOGFILE"
    echo "[INFO] Sending TERM signal to processes..." | tee -a "$LOGFILE"
    /sbin/fuser -kMm -TERM $MOUNTPOINT
    /bin/sleep 5
    echo "[INFO] Sending KILL signal to remaining processes..." | tee -a "$LOGFILE"
    /sbin/fuser -kMm -KILL $MOUNTPOINT
    /bin/sleep 5
    echo "[INFO] Attempting to unmount $MOUNTPOINT" | tee -a "$LOGFILE"
    /bin/umount $MOUNTPOINT
    /bin/sleep 5
done

echo "[SUCCESS] $MOUNTPOINT successfully unmounted `hostname`." | tee -a "$LOGFILE"
EOF
...
echo "[SUCCESS] $MOUNTPOINT successfully unmounted from `hostname`." | tee -a "$LOGFILE"
EOF
